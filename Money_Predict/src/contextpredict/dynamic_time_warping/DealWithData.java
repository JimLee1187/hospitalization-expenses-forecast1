...
public ArrayList<Double> list;
	...
	public double[] regularization(double[] list0){
		double[] list = new double[list0.length];
		...
		mean = sum / list0.length;
		sum = 0.0;
		for(double d : list0){
			sum += Math.pow(d - mean, 2);
		}
		variance = sum/list0.length;
		//System.out.println(variance);
		for(int i = 0; i < list0.length; i++){
			if(variance != 0){
				list1.add((list0[i] - mean)/variance);
		...
		for(int i = 0; i < list1.size(); i++){
			if(max - min != 0){
				list[i] = (list1.get(i) - min)/(max - min);
